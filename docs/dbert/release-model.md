...
After training, the checkpoints are stored in DeepSpeed's sharded format (multiple files per checkpoint). To use the model for inference, you need to convert it to a single-file, HuggingFace-compatible format.

## Conversion Script

```python
from clinical_transformer import dBertPretrainedModelForMVP
from clinical_transformer import dBertTokenizerTabular as dBertTokenizer
import torch
...
tokenizer = dBertTokenizer.from_pretrained(
...
After running this script, `model_path/` will contain everything needed for inference:
...
:::{tip}
Pick the checkpoint epoch that shows the best (lowest) training loss. You can inspect the loss CSV from Step 3 to find it.
:::
