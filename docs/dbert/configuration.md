...
dataset:
  ...
  masking_fraction: 0.15    # Fraction of features to mask
...
model:
  ...
  mask1_ratio: 0.25         # Fraction of masked tokens isolated (dBERT-specific)
  mask1_warmup_epochs: 0    # Epochs to anneal mask1_ratio from 0 (0 = disabled)
  gated_attention: false    # Per-head sigmoid gate after SDPA (experimental)
  dropout: 0.1
...
| `context_window` | How many features the model sees per training step. |
| `masking_fraction` | Fraction of context-window features to mask. 0.15 is standard. |
...
| `intermediate_size` | Feed-forward network hidden size. Typically `4 x hidden_size`. |
| `num_attention_heads` | Number of attention heads per layer. |
| `num_hidden_layers` | Depth of the transformer encoder. |
| `mask1_ratio` | **dBERT-specific.** Fraction of masked tokens that are isolated (identity-only prediction). Set to `0.25` for the recommended default. Set to `0.0` for scGPT-style masking, `-1.0` for standard BERT. |
| `mask1_warmup_epochs` | **dBERT-specific.** If > 0, linearly ramp `mask1_ratio` from 0 to target over this many epochs. Default 0 (no annealing). |
| `gated_attention` | **dBERT-specific.** If `true`, applies a per-head sigmoid gate after SDPA (Qiu et al., 2025). Experimental; no benefit observed at small scale. |
| `dropout` | Dropout rate applied in attention and feed-forward layers. |
| `learning_rate` | Initial learning rate. |
| `optimizer` | `torch.optim.Adam`, `torch.optim.AdamW`, `DeepSpeedCPUAdam`, or `FusedAdam`. |
...
| `stage` | DeepSpeed ZeRO stage. **Stage 2**: shards optimizer states and gradients. **Stage 3**: also shards model parameters. |
