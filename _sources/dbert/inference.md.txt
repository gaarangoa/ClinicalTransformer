...
```python
from clinical_transformer import dBertPretrainedModelForMVP, dBertTokenizerTabular
import torch

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

model_path = "/path/to/model_hub/"
tokenizer = dBertTokenizerTabular.from_pretrained(model_path)
model = dBertPretrainedModelForMVP.from_pretrained(
