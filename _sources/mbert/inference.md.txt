...
Once you have a released model from {doc}`release-model`, load it and extract embeddings from new data.

## Load the Model

```python
from clinical_transformer.mbert import mBertPretrainedModelForMVP, mBertTokenizerTabular
import torch

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

model_path = "/path/to/model_hub/"
tokenizer = mBertTokenizerTabular.from_pretrained(model_path)
model = mBertPretrainedModelForMVP.from_pretrained(model_path).to(device)
model.eval()
```
...
**CLS embedding** -- the representation from the special classification token:
...
**Mean-pooled embedding** -- the average of all feature embeddings (excludes CLS):
