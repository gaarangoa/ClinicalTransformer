...
class MaskedTokenDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        max_pos = len(sample_tokens) - self.context_window + 0
        pix = torch.randint(0, max(1, max_pos), (1,) ).item()
        
        # Extract context window
        tokens = torch.tensor(sample_tokens[pix:pix+self.context_window+1], dtype=torch.long)
        if self.return_values:
            values = torch.tensor(sample_values[pix:pix+self.context_window+1])
        else:
            values = []
