...
class Classifier(lightning.LightningModule):
    def __init__(self, config):
        ...
        self.classifier = torch.nn.Linear(MasterModel.hparams.ninp, config.model.n_classes)        
        self.relu = torch.nn.ReLU()
        self.loss = config.loss.name(reduction='mean')
    ...
    def forward(self, **kwargs):
        ...
        output = self.relu(self.classifier(output_embeddings[:, 0, :])) # takes the [cls] embeddings as the input features
    ...
    def compute_auc(self, y_pred, y_true):
        y_pred = y_pred.detach().cpu().to(torch.float32).numpy()[:, 0]
...
def pipeline():
    ...
    # Loss
    config.loss.name = eval(config.loss.name)
